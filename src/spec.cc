...
phenome::PhenomeParams build_pheno_params(const Simulation& simulation) {
    ...
    if (!pheno_data.effects.has_value())
      pheno_effects[pheno] =
          Eigen::VectorXd::Constant(
              ...
              1.0 / std::sqrt(pheno_data.n_causal_loci));
    else
      pheno_effects[pheno] =
...
Params build_params(const Simulation& simulation) {
  genome::GenomeParams geno = genome::GenomeParams{
      .n_ind = simulation.n_individuals,
      .n_loc = simulation.genome.n_loci,
      .v_rec = expand(simulation.genome.v_rec, simulation.genome.n_loci),
      .v_maf = expand(simulation.genome.v_maf, simulation.genome.n_loci),
      .v_mut = expand(simulation.genome.v_mut, simulation.genome.n_loci)};

  phenome::PhenomeParams pheno = build_pheno_params(simulation);

  mating::MatingParams mate = mating::MatingParams{
